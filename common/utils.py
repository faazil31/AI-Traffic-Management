...
def display_result(wait_time,lanes):
    ...
    """
    hori_image = cv2.hconcat((img1, img2))
    hori2_image = cv2.hconcat((img3, img4))
    all_lanes_image = cv2.vconcat((hori_image, hori2_image))"""
...
def drawPred( frame, classId, conf, left, top, right, bottom):
        ...
        cv2.rectangle(frame, (top,bottom), (left, right), color=(0, 255, 0), thickness=2)
        return frame

        # Draw a bounding box.
        cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), thickness=3)
